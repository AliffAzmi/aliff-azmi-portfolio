...
<section>
	<!-- transition:fly|local={{ x: 400, duration: 3000 }} -->
	<div 
	class="mx-auto max-w-screen-xl">
		{#if isHome}
			<div class="flex items-center">
				<div class="flex-1">
					<h1 class="text-xl md:text-4xl font-semibold text-center lg:text-left">
						Latest Articles
					</h1>
				</div>
			</div>
		...
		<div class="article-container">
			{#each blogs as article}
				<div class="articles-box">
					<div class="articles-header">
						...
						<div class="articles-tag">
							{#each article.tags as tag}
								<div class="text-sm text-teal-500 hover:text-teal-300">{tag}</div>
							{/each}
						</div>
					</div>
					...
					>
						<a href={`/blog/${paramCase(article.title)}`} on:click={(e) => id.set(article.id)}
							>Read articles =></a
						></button
					>
				</div>
